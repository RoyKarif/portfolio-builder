...
def test_basic_optimization():
    ...
    result = optimize_portfolio(tickers=tickers, expected_returns=expected_returns, cov_matrix=cov_matrix, risk_level=3, target_return=0.10)

    assert len(result["weights"]) == 5
    assert abs(sum(result["weights"].values()) - 1.0) < 0.01
    assert all(w >= 0 for w in result["weights"].values())
    assert all(w <= 0.30 for w in result["weights"].values())
